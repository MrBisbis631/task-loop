...
const LocationSelector = ({ disabled, onCountryChange, onStateChange, defaultValue }: LocationSelectorProps) => {
  const [selectedCountry, setSelectedCountry] = useState<CountryProps | null>(defaultValue?.country || null);
  const [selectedState, setSelectedState] = useState<StateProps | null>(defaultValue?.state || null);
    ...
    <div className="flex gap-4">
      ...
      {availableStates.length > 0 && (
        <Popover open={openStateDropdown} onOpenChange={setOpenStateDropdown}>
          <PopoverTrigger asChild>
            <Button variant="outline" role="combobox" aria-expanded={openStateDropdown} disabled={!selectedCountry} className="w-full justify-between">
