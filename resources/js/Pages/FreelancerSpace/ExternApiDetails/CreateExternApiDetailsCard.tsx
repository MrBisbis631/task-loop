...
const CreateExternalApiDetailsFormSchema = z
  .object({
  ...
  return (
    <Card className="bg-slate-50">
      <CardHeader>
        ...
        <CardDescription>Add new API keys to access external services, such as payment gateways, email services, etc.</CardDescription>
      </CardHeader>
      <CardContent>
          ...
          <DialogContent className="sm:max-w-[625px]">
            <form action="" onSubmit={onSubmit}>
              <DialogHeader>
                ...
                <DialogDescription>Fill the form and submit your new API key</DialogDescription>
              </DialogHeader>
              <div className="grid grid-cols-2 gap-4 py-4">
                ...
                <div className="max-sm:col-span-2">
                  <Label htmlFor="api_token" className="block font-bold capitalize text-slate-800">
                    API token
                  </Label>
                  <Label htmlFor="api_token" className="block text-slate-500 text-xs mb-1">
                    Token given by the API for authentication
                  </Label>
                  <Input {...form.register("api_token")} type="password" autoComplete="off" placeholder="API Token" />
                  {!!errors.api_token ? (
                    <Label htmlFor="api_token" aria-description="error message" className="block text-red-500 mt-1">
                      {errors.api_token.message}
                    </Label>
                  ) : null}
                </div>
                <div className="max-sm:col-span-2">
                  <Label htmlFor="api_secret" className="block font-bold capitalize text-slate-800">
                    API secret
                  </Label>
                  <Label htmlFor="api_secret" className="block text-slate-500 text-xs mb-1">
                    Secret token provided by the API (if given)
                  </Label>
                  <Input {...form.register("api_secret")} type="password" autoComplete="off" placeholder="API Secret" />
                  {!!errors.api_secret ? (
                    <Label htmlFor="api_secret" aria-description="error message" className="block text-red-500 mt-1">
      ...
      <CardFooter className="text-xs space-x-1 text-muted-foreground">
        ...
        <span>Your API keys are securely stored and encrypted.</span>
      </CardFooter>
    </Card>
  );
}
